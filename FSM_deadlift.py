...
import cv2
import numpy as np
from utils import find_angle, find_angle_abs, get_landmark_features, draw_text, draw_dotted_line
...
class FSM_deadlift:
    ...
    def __init__(self, thresholds, flip_frame=False, pre_trained=True):
        ...
        self.state_tracker = {
                                ...
                                # 0 --> Bend Backwards, 1 --> Bend Forward, 2 --> Keep shin straight, 3 --> Deep DEADLIFT
                                "DISPLAY_TEXT": np.full((4,), False),
                                "COUNT_FRAMES": np.zeros((4,), dtype=np.int64),

                                "VALID_DEADLIFT": False,
                                "LOWER_HIPS": False,
                                "NO LIFT": False,
        ...
        self.FEEDBACK_ID_MAP = {
                                ...
                                3: ('DEADLIFT TOO DEEP', 125, (255, 80, 80))
        }
    ...
    def _get_state(self, hip_angle, knee_angle):
        """Get the state of the deadlift.


        Args:
            hip_angle (int): Angle between the line hip-knee and a vertical.
        ...
        if (self.thresholds['HIP_KNEE_VERT']['START'] <= hip_angle) and \
            (self.thresholds['KNEE_HIP_VERT']['START'][0] <= knee_angle <= self.thresholds['KNEE_HIP_VERT']['START'][1]):
            hip = 1

        elif self.thresholds['HIP_KNEE_VERT']['TRANS'][0] <= hip_angle <= self.thresholds['HIP_KNEE_VERT']['TRANS'][1]:
            hip = 2
        elif hip_angle <= self.thresholds['HIP_KNEE_VERT']['EXTEND']:
            hip = 3

        return f's{hip}' if hip else None         
    
    def _show_state_tracker_info(self):
        return (
            f"""SEQUENCE {self.state_tracker['state_seq']},
                CURRENT STATE {self.state_tracker['curr_state']},
                PREVIOUS STATE {self.state_tracker['prev_state']}

                COUNTERS: {self.state_tracker['DEADLIFT_COUNT'], self.state_tracker['IMPROPER_DEADLIFT']}
                """
        )
    ...
    def _check_movement(self):
            ...
            if self.state_tracker['state_seq'] == self.valid_dl['state_seq']:
                return True
            else:
                return False
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                self.state_tracker['DEADLIFT_COUNT'] = 0
                self.state_tracker['IMPROPER_DEADLIFT'] = 0
            ...
            # Camera is aligned properly
                ...
                hip_vertical_angle = find_angle(shldr_coord, np.array([hip_coord[0], 0]), hip_coord) * -multiplier
                ...
                knee_vertical_angle = find_angle_abs(hip_coord, np.array([knee_coord[0], 0]), knee_coord)
                ...
                current_state = self._get_state(hip_vertical_angle, knee_vertical_angle)
                ...
                # ----------------------------------------------------------------------------------------------------
                    ...
                    # if self.thresholds['HIP_THRESH'][1] > hip_vertical_angle and \
                    #     self.state_tracker['state_seq'].count('s2')==1:
                        
                    #     self.state_tracker['NO LIFT'] = True

                    # elif self.thresholds['HIP_THRESH'][0] < hip_vertical_angle:
                    #     self.state_tracker['NO LIFT'] = False
                    pass
                ...
                if self.state_tracker['DEADLIFT_COUNT'] > 0 or self.state_tracker['IMPROPER_DEADLIFT'] > 0:
                
                    frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['VALID_DEADLIFT'])
            ...
            self.state_tracker['DEADLIFT_COUNT'] = 0
            self.state_tracker['IMPROPER_DEADLIFT'] = 0
                # cv2.putText(frame, 'Resetting DEADLIFT_COUNT due to inactivity!!!', (10, frame_height - 25), self.font, 0.7, self.COLORS['blue'], 2)
